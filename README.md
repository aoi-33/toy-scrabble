...
3. COM 対戦時、相手の手番中は "🤖 COM 思考中…" が表示され、Rack と ActionBar がロックされます
4. 手札のタイルをクリック → 空マスをクリック、または手札をドラッグしてマスに置く
5. すべてのタイルを置いたら `PLAY` を押す（259,416 語の英単語リストで検証）
6. `RECALL` で全ての pending タイルを手札に戻す
7. `EXCHANGE` で手札の一部を袋のタイルと入れ替える（袋残 7 枚以上のとき）
8. `PASS` でターンをスキップ（6 連続で終了）
9. どちらかがタイルを使い切って袋が空、または 6 連続 Pass でゲーム終了
10. 「直前手」と「履歴」に出る単語をタップすると、発音記号（IPA）と英英定義（品詞ごとに 1 件）、和訳（最大 3 件）が表示されます
...
- **2026-09-12** ホーム画面から起動したときに、タイトルや `?` ボタンがノッチ（カメラ部）と重なるのを直した。`viewport-fit=cover` で webview がステータスバーの下まで広がるため、上端に `env(safe-area-inset-top)` ぶんの余白を取るようにした。
