...
- `docs/design.md`: **更新しない。** 同ファイルは Plan 1〜4 当時の設計文書で、辞書アセットを `public/dict/en-en/{a..z}.json` と `en-ja/{a..z}.json` に分ける旧構成のまま止まっており、現行のバケット形式（`b`/`e`/`j`）を記述した箇所が無い。追記すると旧構成の記述と矛盾する。バケット形式の正典は本書 §3。ライセンス表も、発音記号は既存の Wiktionary 由来で新しいデータ源ではないため行は増えない
